...
OpenClaw Fleet connects to your [OpenClaw Gateway](https://github.com/kddige/openclaw-gateway) instances over WebSocket. You can connect to as many gateways as you need — production, staging, development, all at once.
...
The first time you connect to a gateway, your device needs to be approved. OpenClaw Fleet uses Ed25519 keypairs for authentication — no passwords.

1. Fleet sends a pairing request to the gateway
2. Approve the device via the **OpenClaw Gateway Control UI > Devices tab**
3. Once approved, Fleet connects automatically on future launches
