...
All logging in `electron/` must go through the `Logger` interface from `electron/logger/`. Direct `console.*` calls are blocked by ESLint. See [Logging](/docs/developers/logging) for full details.
